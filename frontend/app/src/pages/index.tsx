      ...
      <div className="flex flex-col mt-5 w-full items-center justify-start">
        ...
        <Tabs defaultValue="email" className="w-[80%]">
          <TabsList className="grid w-full grid-cols-3">
            {searchKeys.map((item) => (
              <TabsTrigger onClick={clearSearchIput} key={item.value} value={item.value}>
                {item.title}
              </TabsTrigger>
            ))}
          </TabsList>
          {searchKeys.map((item) => (
            <TabsContent key={item.value} value={item.value}>
              <Card>
                ...
                <form onSubmit={item.submitFunc} className="w-full">
                          ...
                          <div className="flex flex-col w-full">
                            {breaches.map((breach) => (
                              ...
                              >
                                ...
                                <p>{`Fecha de subida: ${breach.created_at.slice(
                                  0,
                                  10
                                )}`}</p>
                              </div>
